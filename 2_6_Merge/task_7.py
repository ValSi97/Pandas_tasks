...
Получите DF, показывающий сколько каждый клиент потратил в первом и втором магазине. Обратите внимание на метки столбцов.
...
shop_1 = (shop_1[['client', 'price']].groupby('client', as_index=False).agg('sum').rename(columns={'price': 'sum_shop_1'})
          .astype({'sum_shop_1': float}))
shop_2 = (shop_2[['client', 'price']].groupby('client', as_index=False).agg('sum').rename(columns={'price': 'sum_shop_2'})
          .astype({'sum_shop_2': float}))
new_df = shop_1.merge(shop_2, how='outer')
