...
В столбце 'product_price' замените строки в формате 'xxxx руб' на число xxxx.
...
df = pd.DataFrame(data={'product': ['Товар_' + str(rd.choice(range(1, 10))) for i in range(100)],
                        'product_price': [str(rd.choice(range(1000, 10000, 1000))) + ' руб' for i in range(100)],
                        'count': [rd.choice(range(1, 10)) for i in range(100)],
                        'buy': [rd.choice(['куплено', 'не куплено']) for i in range(100)]
                        }
                  )

df['product_price'] = df['product_price'].map(lambda x: int(x.split(' ')[0]), na_action='ignore')
print(df)
